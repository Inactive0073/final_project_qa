...
class ProductPage(BasePage):
    ...
    def should_be_btn_add_in_cart(self):
        assert self.is_element_present(
            *CardPageLocators.PRODUCT_BTN
        ), "The button is not present"

    def should_be_success_msg_about_addition(self):
        assert self.is_element_present(
            *CardPageLocators.PRODUCT_MSG_TITLE
        ), "The message about addiition to the cart is not present"

    def title_should_be_the_same(self):
        assert (
            self.browser.find_element(*CardPageLocators.PRODUCT_TITLE).text
            == self.browser.find_element(*CardPageLocators.PRODUCT_MSG_TITLE).text
        )

    def price_should_be_the_same(self):
        assert (
            self.browser.find_element(*CardPageLocators.PRODUCT_PRICE).text
            == self.browser.find_element(*CardPageLocators.PRODUCT_MSG_PRICE).text
        )
