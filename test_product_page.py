...
class TestUserAddToBasketFromProductPage:
    ...
    def setup(self, browser):
        page = LoginPage(
            browser, "http://selenium1py.pythonanywhere.com/en-gb/accounts/login/"
        )
        page.open()
...
xlink = pytest.param(
    f"{mask}{xfile}", marks=pytest.mark.xfail(reason=f"mistake on page {mask}{xfile}")
)
...
def test_guest_cant_see_success_message_after_adding_product_to_basket(
    browser, link="http://selenium1py.pythonanywhere.com/catalogue/coders-at-work_207/"
):
    product_page = ProductPage(browser, link)
    product_page.open()
    product_page.click_to_add_to_the_cart_button()
